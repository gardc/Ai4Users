...
const ConsentButton: React.FC<ConsentButtonProps> = ({
    ...
    return (
        <div className={"flex"}>
            <Link href={href} className={"bg-prussian-blue hover:bg-opacity-70 font-semibold text-white rounded-2xl text-center m-2 sm:m-3 px-6 py-2 sm:py-3 w-56 flex items-center justify-center"}>
                {children}
            </Link>
        </div>
    );
